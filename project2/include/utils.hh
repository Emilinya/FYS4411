...
using Mat = arma::dmat::fixed<N, M>;

template <size_t N, size_t d, size_t M>
struct RBMParams
{
    Vec<M> b = Vec<M>(arma::fill::randn);
    Vec<N *d> a = Vec<N * d>(arma::fill::randn);
    Mat<N * d, M> W = Mat<N * d, M>(arma::fill::randn);
};

template <size_t N, size_t d, size_t M>
struct RBMGrad
{
    Vec<M> bGrad = Vec<M>(arma::fill::zeros);
    Vec<N *d> aGrad = Vec<N * d>(arma::fill::zeros);
    Mat<N * d, M> WGrad = Mat<N * d, M>(arma::fill::zeros);
};
...
// open file with error checking (why is this not default!)
std::optional<std::ofstream> errcheckOpen(std::string filename) {
